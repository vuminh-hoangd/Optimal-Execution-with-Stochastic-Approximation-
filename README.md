...
```math
r^* \in \arg\max_{r \in \mathcal{P}_N} \sum_{i=1}^{N} \rho_i \mathbb{E}[\min(r_i V, D_i)],\qquad \mathcal{P}_N := \{r \in \mathbb{R}_+^N : \sum_{i=1}^N r_i = 1\}.
```
...
This is a zero of an expectation that can be **sampled** (one realized fill/no-fill per trading period) even though the distribution of $Y=(V,D_1,\dots,D_N)$ is never known.
...
with $\Pi_{\mathcal{P}_N}$ the projection onto the simplex $\mathcal{P}_N$. Under some regular conditions and $\sum\gamma_k=\infty$, $\sum\gamma_k^2<\infty$: 
 
$$ r^{(k)} \xrightarrow[k \to \infty]{\text{a.s.}} r^* $$

## 3. Experimental Results

Calibrated on real Binance market data benchmarked against an oracle with perfect foresight (insider trading).
